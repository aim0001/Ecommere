...
class CartController extends AbstractController
{
    ...
    #[Route('/cart/increment/{id}', name: 'app_cart_increment', methods: ['GET'])]
    public function increment(SessionInterface $session, Product $product): JsonResponse
    {
        ...
        // Retourne une réponse JSON (optionnel, selon les besoins)
        return new JsonResponse(['status' => 'success', 'cart' => $cart]);
    }

    #[Route('/cart/decrement/{id}', name: 'app_cart_decrement', methods: ['GET'])]
    public function decrement(SessionInterface $session, Product $product): JsonResponse
    {
        // Récupère le panier de la session
        $cart = $session->get('cart', []);
        ...
        if (isset($cart[$product->getId()])) {
            $cart[$product->getId()]--;
            // Si la quantité atteint 0, supprime le produit du panier
